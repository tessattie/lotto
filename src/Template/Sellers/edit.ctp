...
?>
    ...
    <div class="row">
        <div class="col-md-12">
            <h3 style="padding-bottom:10px">Editer le Vendeur : <?= $seller->first_name." ".$seller->last_name ?></h3>
        </div>
    </div>
    <?= $this->Form->create($seller, ['enctype' => 'multipart/form-data']) ?>
    <fieldset>
        <?php
        ...
        <div class="form-group">
          ...
          <?php 
            echo $this->Form->control('manager_id', ['options' => $managers, "label" => "Responsable", "empty" => " -- Choisissez -- "]);
            echo $this->Form->control('bank_id', ['options' => $banks, "label" => "Banque", "empty" => " -- Choisissez -- "]);
           ?>
    </fieldset>
